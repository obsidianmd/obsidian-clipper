...
- **WebDAV** reads and writes the normalized payload as a JSON file at a user-configured URL. It supports optional HTTP Basic authentication and keeps a local last-known-good cache for temporary network failures.
...
- WebDAV currently supports a direct HTTP(S) file URL and optional Basic authentication. It does not create missing parent directories, negotiate OAuth, support client certificates, or retry failed writes.
...
- WebDAV credentials are stored in extension-local storage. Browser extension storage is not a secure system keychain, so an app password with narrowly scoped access is recommended.
