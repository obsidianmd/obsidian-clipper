...
export function matchBrowserLanguage(): string {
	const browserLang = browser.i18n
		.getUILanguage()
		.toLowerCase()
		.split("-")[0]; // Get base language code
...
export function getMessage(
	...
	try {
		...
		console.warn(
			`Failed to load messages for language ${currentLanguage}`,
			error
		);
		return (
			browser.i18n.getMessage(messageName, substitutions) || messageName
		);
	}
}
...
export async function getEffectiveLanguage(): Promise<{
	code: string;
	isRTL: boolean;
}> {
