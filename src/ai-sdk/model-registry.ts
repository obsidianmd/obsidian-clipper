...
function buildFallbackProviders(): ModelsDevApiResponse {
	...
	for (const [providerId, preset] of Object.entries(presets)) {
		...
		const models: Record<string, ModelsDevApiResponse[string]['models'][string]> = {};
		
		for (const [modelId, modelData] of Object.entries(preset.fallbackModels)) {
			models[modelId] = {
				id: modelId,
				name: modelData.name,
				...
				limit: modelData.limit ?? { context: DEFAULT_CONTEXT_LIMIT, output: DEFAULT_OUTPUT_LIMIT },
				cost: modelData.cost
			};
		}
	...
	try {
		...
		if (storedCache && Date.now() - storedCache.timestamp < CACHE_TTL_MS) {
			...
			debugLog('ModelRegistry', 'Loaded from cache', {
				...
				age: Math.round((Date.now() - cacheTimestamp) / 1000 / 60) + ' minutes'
			});
			return;
		}
...
export async function refreshRegistry(): Promise<void> {
	try {
		...
		if (!parseResult.success) {
			console.error('Invalid models.dev API response format:', parseResult.error.issues);
		...
		if (!cachedData) {
			cachedData = getFallbackProviders();
			debugLog('ModelRegistry', 'Using fallback data after fetch failure');
		}
	}
}
...
function mapApiModelToCapabilities(
	...
	return {
		...
		modalities: {
			input: (model.modalities?.input ?? ['text']) as ('text' | 'image' | 'pdf' | 'audio')[],
			output: (model.modalities?.output ?? ['text']) as ('text' | 'image' | 'audio')[]
		},
		limit: {
 ...
 */
export function getModel(providerId: string, modelId: string): ModelCapabilities | null {
	if (!cachedData) {
 ...
 */
export function validateContextSize(providerId: string, modelId: string, tokenCount: number): boolean {
 ...
 */
export function supportsStructuredOutput(providerId: string, modelId: string): boolean {
	const model = getModel(providerId, modelId);
	...
	if (!model) {
		debugLog('ModelRegistry', 'Model not found, using text fallback (conservative)', {
			providerId,
			modelId
		});
		return false;
 ...
 */
export function getModelCapabilityHints(providerId: string, modelId: string): ModelCapabilityHints {
	const model = getModel(providerId, modelId);
