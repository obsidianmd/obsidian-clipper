...
import { loadSyncPayload, repairSelectedSyncProvider, updateSyncPayload } from '../sync/sync-manager';
...
export async function loadTemplates(): Promise<Template[]> {
	try {
		...
		templates = payload?.templates.filter(template => template && Array.isArray(template.properties)) ?? [];
...
export async function saveTemplateSettings(): Promise<string[]> {
	...
	try {
		const result = await updateSyncPayload(payload => ({ ...payload, templates }));
...
export async function rebuildTemplateList(): Promise<void> {
	try {
		const payload = await repairSelectedSyncProvider();
