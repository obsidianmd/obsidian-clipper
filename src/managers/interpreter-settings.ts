...
export function updatePromptContextVisibility(): void {
	...
	if (promptContextContainer) {
		promptContextContainer.style.display = interpreterToggle.checked
			? "block"
			: "none";
	}
...
async function showProviderModal(provider: Provider, index?: number) {
	...
	if (form) {
		...
		const updateVisibility = () => {
			...
			const selectedPreset = selectedPresetId
				? (cachedPresetProviders || {})[selectedPresetId]
				: null;
			...
			if (selectedPreset) {
				...
				const editingOriginalPreset =
					index !== undefined && selectedPresetId === currentPresetId;
				baseUrlInput.value = editingOriginalPreset
					? provider.baseUrl
					: selectedPreset.baseUrl;
				apiKeyInput.value = editingOriginalPreset
					? provider.apiKey
					: "";

				apiKeyContainer.style.display =
					selectedPreset.apiKeyRequired === false ? "none" : "block";

				if (
					selectedPreset.apiKeyRequired !== false &&
				...
				if (
					index === undefined ||
					(index !== undefined && currentPresetId)
				) {
					nameInput.value = "";
					baseUrlInput.value = "";
					apiKeyInput.value = "";
	...
	newConfirmBtn?.addEventListener("click", async () => {
		...
		const presetId = (
			form.querySelector('[name="preset"]') as HTMLSelectElement
		).value;
		...
		if (
			presetId &&
			cachedPresetProviders &&
			cachedPresetProviders[presetId]
		) {
			...
			updatedProvider.baseUrl =
				baseUrl !== providerPresetBaseUrl
					? baseUrl
					: providerPresetBaseUrl;
			updatedProvider.apiKeyRequired =
				providerPreset.apiKeyRequired !== false;
		}
...
async function showModelModal(model: ModelConfig, index?: number) {
	...
	if (form) {
		...
		const updateModelOptions = () => {
			...
			nameInput.value =
				index !== undefined && model.providerId === selectedProviderId
					? model.name
					: "";
			providerModelIdInput.value =
				index !== undefined && model.providerId === selectedProviderId
			...
			if (provider && cachedPresetProviders) {
				...
				if (presetProvider?.modelsList) {
					modelIdDescriptionContainer.textContent =
						getMessage("providerModelIdDescription") + " ";
					const linkElement = document.createElement("a");
				...
				if (presetProvider?.popularModels?.length) {
					...
					modelSelectionRadios.addEventListener("change", (e) => {
						...
						if (!target || target.name !== "model-selection")
							return;
		...
		newConfirmBtn.addEventListener("click", async () => {
			...
			if (
				!updatedModel.name ||
				!updatedModel.providerId ||
				!updatedModel.providerModelId
			) {
